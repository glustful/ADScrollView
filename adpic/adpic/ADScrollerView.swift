...
class ADScrollerView: UIView,UIScrollViewDelegate {
    ...
    var imageLinkURL:[String]?{
        ...
        didSet{
            ...
            //初始化图片控件
            leftImageView!.load(self.imageLinkURL![leftImageIndex],placeholder:placeHoldImage)
            
            centerImageView!.load(self.imageLinkURL![centerImageIndex],placeholder:placeHoldImage)
            rightImageView!.load(self.imageLinkURL![rightImageIndex],placeholder:placeHoldImage)
        }
    ...
    //加载时的占位图片
    var placeHoldImage = UIImage(named: "placeHolder.jpg")
    //设置是否需求定时滚动
    ...
    override init(frame: CGRect) {
        ...
        adMoveTime = 2.0
        var childFram = CGRectMake(0, 0, frame.size.width, frame.size.height)
        adScrollView = UIScrollView(frame: childFram)
