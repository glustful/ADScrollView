    ...
    var imageLinkURL:[String]?{
        ...
        didSet{
            ...
            for url in self.imageLinkURL!{
                ...
                imageView.load(url,placeholder:placeHoldImage)
            ...
            if self.imageLinkURL?.count>1{
                ...
                imageView.load((self.imageLinkURL?.last)!,placeholder:placeHoldImage)
                adScrollView!.addSubview(imageView)
                
                imageView = UIImageView(frame: CGRectMake(kADWidth!*index, 0, kADWidth!, kADHeight!))
                
                imageView.load((self.imageLinkURL?.first as String?)!,placeholder:placeHoldImage)
                adScrollView!.addSubview(imageView)
            }
